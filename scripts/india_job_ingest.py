#!/usr/bin/env python3
"""Stable India job discovery: direct boards + Google indexed board results."""
...
from google_job_discovery import discover_google
...
def normalize(row,max_age_hours):
    title=clean(row.get("title"));company=clean(row.get("company"));location=clean(row.get("location")) or clean(row.get("city"));url=clean(row.get("job_url",row.get("apply_url",row.get("url"))));source=(clean(row.get("site",row.get("source"))) or "").lower().strip()
    ...
    try:
        posted_dt=pd.to_datetime(raw_posted,utc=True); age=(pd.Timestamp.now(tz="UTC")-posted_dt).total_seconds()/3600
        if pd.isna(posted_dt) or age < -1 or age > max_age_hours:return None
    ...
    for name,fn in (("foundit",scrape_foundit),("naukri",scrape_naukri)):
        try:
            for raw in fn():
                ...
                if job:collected.append(job);source_counts[name]+=1
        except Exception as exc:failures.append(f"{name}: {type(exc).__name__}: {exc}")
    try:
        for raw in discover_google():
            job=normalize(raw,hours_old)
            if job:collected.append(job);google_counts[job["source"]]+=1
    except Exception as exc:failures.append(f"google: {type(exc).__name__}: {exc}")
    unique={}
