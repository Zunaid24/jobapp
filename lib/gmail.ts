...
export async function getGmailConnection(sessionId: string): Promise<GmailConnection | null> {
  const db = supabaseAdmin();
  const { data, error } = await db.from("gmail_connections").select("*").eq("session_id", sessionId).maybeSingle();
  if (error) throw error;
  if (!data) return null;

  const sessionExpiresAtMs = new Date(data.session_expires_at).getTime();
  if (sessionExpiresAtMs <= Date.now()) {
    await db.from("gmail_connections").delete().eq("session_id", sessionId);
    return null;
  }

  let accessToken = decryptSecret(data.access_token_encrypted);
  let accessTokenExpiresAt = new Date(data.access_token_expires_at).getTime();
  const refreshToken = data.refresh_token_encrypted ? decryptSecret(data.refresh_token_encrypted) : null;

  if (accessTokenExpiresAt <= Date.now() + 60_000) {
...
export async function disconnectGmail(sessionId: string) {
  const db = supabaseAdmin();
  const { error } = await db.from("gmail_connections").delete().eq("session_id", sessionId);
  if (error) throw error;
}
