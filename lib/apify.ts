...
const DEFAULT_INPUT = { keyword: "HR", location: "Goa", experience: 0, maxResults: 12, sources: ["LinkedIn", "Indeed", "SimplyHired"], postedMaxDays: 2, jobType: "Any", educationLevel: "Any", skills: "" };
...
function normalize(job:ApifyJob){
 ...
 const parsed=posted?new Date(posted):null;if(parsed&&!Number.isNaN(parsed.getTime())){const age=(Date.now()-parsed.getTime())/86400000;if(age< -0.5||age>2)return null;}
 ...
 const companyWebsite=text(job.companyWebsite,job.company_website,job.website,job.companyUrl,job.company_url,job.employerWebsite,job.employer_website,nestedCompanyValue(job,"website","url","websiteUrl","companyWebsite"))||null;
 const companyLinkedin=text(job.companyLinkedin,job.company_linkedin,job.companyLinkedinUrl,job.company_linkedin_url,nestedCompanyValue(job,"linkedin","linkedinUrl","linkedin_url"))||null;
 let domain="";try{if(companyWebsite){const u=new URL(companyWebsite.includes("://")?companyWebsite:`https://${companyWebsite}`);domain=u.hostname.replace(/^www\./,"").toLowerCase();}}catch{}
 return {id,title,company,location:"Goa",type:text(job.type,job.jobType,job.employmentType)||"Full-time",match_score:HR_JOB_TITLE.test(title)?100:85,description:description.slice(0,30000),apply_url:url||null,contact_email:null,decision_maker_name:null,decision_maker_title:null,source:text(job.source,job.platform)||"Apify",posted_at:parsed&&!Number.isNaN(parsed.getTime())?parsed.toISOString():null,company_website:companyWebsite,company_domain:domain||null,company_linkedin_url:companyLinkedin,company_location:text(job.companyLocation,job.company_location,job.headquarters,nestedCompanyValue(job,"location","headquarters"))||null,company_industry:text(job.companyIndustry,job.industry,nestedCompanyValue(job,"industry"))||null,raw:job};
}
...
export async function refreshDailyJobs(options:{force?:boolean}={}){
 ...
 try{
  ...
  const companyRows=Array.from(new Map(selectedWithScores.map(j=>[j.company.toLowerCase(),j])).values());const companyMap=new Map<string,string>();
  for(const job of companyRows){const domain=job.company_domain||null;let query=client.from("companies").select("id").limit(1);query=domain?query.eq("domain",domain):query.is("domain",null).eq("name",job.company);const existing=await query.maybeSingle();let companyId=existing.data?.id;if(!companyId){const {data,error}=await client.from("companies").insert({name:job.company,website:job.company_website,domain,linkedin_url:job.company_linkedin_url,location:job.company_location,industry:job.company_industry,source:job.source,raw:job.raw}).select("id").single();if(error)throw new Error(`Unable to store company: ${error.message}`);companyId=data.id;}else await client.from("companies").update({website:job.company_website,linkedin_url:job.company_linkedin_url,location:job.company_location,industry:job.company_industry,source:job.source,raw:job.raw,updated_at:new Date().toISOString()}).eq("id",companyId);companyMap.set(job.company.toLowerCase(),companyId);}
  if(selectedWithScores.length){const rows=selectedWithScores.map(job=>({id:job.id,title:job.title,company:job.company,location:job.location,type:job.type,match_score:job.match_score,description:job.description,apply_url:job.apply_url,contact_email:null,decision_maker_name:null,decision_maker_title:null,source:job.source,posted_at:job.posted_at,collected_on:today,company_id:companyMap.get(job.company.toLowerCase())||null,raw:job.raw}));const {error}=await client.from("jobs").upsert(rows,{onConflict:"id"});if(error)throw new Error(`Unable to store jobs: ${error.message}`);}
  const companies=Array.from(new Set(selectedWithScores.map(j=>j.company.toLowerCase()))).length;
  ...
  return {skipped:false,itemCount:selectedWithScores.length,goaCount:selectedWithScores.length,companies,decisionMakers:0,seenExcluded:seen.size,apifyCandidates:normalized.length,aiAccepted:selectedWithScores.length,aiModel:process.env.GEMINI_JOB_MODEL||"gemini-2.5-flash-lite"};
