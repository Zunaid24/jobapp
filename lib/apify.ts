...
function isRelevantHrRole(title: string, description: string) {
...
function nestedCompanyValue(job: ApifyJob, ...keys: string[]) {
  const company = job.company;
  if (!company || typeof company !== "object" || Array.isArray(company)) return "";
  const record = company as Record<string, unknown>;
  return text(...keys.map((key) => record[key]));
}

function normalize(job: ApifyJob) {
  const title = text(job.title, job.jobTitle, job.position);
  const company = text(job.company, job.companyName, job.organization, nestedCompanyValue(job, "name", "companyName"));
  const location = text(job.location, job.jobLocation, job.city, job.workplaceType) || "Goa";
  ...
  const companyWebsite = text(
    ...
    nestedCompanyValue(job, "website", "url", "websiteUrl", "companyWebsite"),
  ) || null;
  const companyLinkedin = text(
    job.companyLinkedin,
    job.company_linkedin,
    job.companyLinkedinUrl,
    job.company_linkedin_url,
    nestedCompanyValue(job, "linkedin", "linkedinUrl", "linkedin_url"),
  ) || null;
  ...
  return {
    ...
    source: text(job.source, job.platform) || "Apify",
    ...
    company_location: text(job.companyLocation, job.company_location, job.headquarters, nestedCompanyValue(job, "location", "headquarters")) || null,
    company_industry: text(job.companyIndustry, job.industry, nestedCompanyValue(job, "industry")) || null,
    raw: job,
  };
}
...
async function runActor(input: Record<string, unknown>) {
  ...
  const response = await fetch(`https://api.apify.com/v2/actors/${encodeURIComponent(ACTOR_ID)}/run-sync-get-dataset-items?maxItems=20&maxTotalChargeUsd=0.50`, {
    method: "POST",
...
export async function refreshDailyJobs(options: { force?: boolean } = {}) {
  ...
  try {
    ...
    return { skipped: false, itemCount: selected.length, goaCount: selected.length, companies: uniqueCompanies.length, decisionMakers: decisionMakerCount };
  } catch (error) {
