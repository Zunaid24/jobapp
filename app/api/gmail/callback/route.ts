...
export async function GET(request: Request) {
  ...
  if (error || !code || !state) return NextResponse.redirect(new URL(`/?gmail=${error ? "denied" : "error"}`, request.url));
  ...
  try {
    ...
    if (!sessionId || sessionId !== verified.sessionId) return NextResponse.redirect(new URL("/?gmail=invalid_session", request.url));

    const token = await exchangeCode(code, new URL(request.url).origin);
    const email = await fetchGoogleEmail(token.access_token);
    await saveGmailConnection({ sessionId, accessToken: token.access_token, refreshToken: token.refresh_token, expiresIn: token.expires_in, email });
