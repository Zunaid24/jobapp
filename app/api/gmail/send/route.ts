...
export async function POST(request: Request) {
  try {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recipient)) return NextResponse.json({ error: "Invalid recipient email" }, { status: 400 });

    const { data: profile, error: profileError } = await db().from("candidate_profiles").select("cv_path,cv_name").eq("session_id", sessionId).maybeSingle();
    if (profileError) throw profileError;
    if (!profile?.cv_path) return NextResponse.json({ error: "Upload a PDF CV before sending." }, { status: 400 });
    ...
    const attachment = Buffer.from(await cv.arrayBuffer());
    const filename = (profile.cv_name || "CV.pdf").replace(/[^a-zA-Z0-9._-]/g, "_");
    const boundary = `JobAppBoundary_${crypto.randomUUID().replaceAll("-", "")}`;
    const message = [
      `To: ${recipient}`,
      "MIME-Version: 1.0",
    ...
    return NextResponse.json({ sent: true, attachment: filename });
  } catch (error) {
