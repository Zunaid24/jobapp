...
export async function GET(request:Request){
  try{
    ...
    const beforeQualityCount=beforeJobs.filter(displayable).length;
    if(location==="Goa"&&beforeQualityCount<5){try{console.info("[JobApp] Discovery starting",{beforeCount:beforeJobs.length,beforeQualityCount,cutoff,reason:"fewer than 5 displayable Goa jobs"});const result=await runGoaDiscovery({force:true});discovery=result as Record<string,unknown>;const refreshed=await queryJobs(client,location,cutoff);if(refreshed.error)console.error("[JobApp] Post-discovery Supabase query failed",refreshed.error);else afterJobs=refreshed.data??beforeJobs;}catch(error){console.error("[JobApp] On-demand Goa discovery failed",error);discovery={error:error instanceof Error?error.message:"Discovery failed"};}}
